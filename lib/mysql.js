...
class DB {
	...
	fetchAll(tableName, selectStr, whereJson, orderByJson = "", limitArr = "") {
		...
		const likeWhere = whereJson["like"];
		const andArr = [];
		const orArr = [];
		const likeArr = [];
		...
		// 新增格式化条件格式
		if (andWhere) {
			andWhere.forEach((item) => {
				const snap = typeof item.key === "string" ? '"' : "";
				const mark = item.mark ? item.mark : "=";
				const key = item.key === "null" ? "" : `${snap}${item.key}${snap}`;
				andArr.push(`\`${item.name}\` ${mark} ${key}`);
			});
		}
		...
		if (orWhere) {
			orWhere.forEach((item) => {
				const snap = typeof item.key === "string" ? '"' : "";
				const mark = item.mark ? item.mark : "=";
				orArr.push(`\`${item.name}\` ${mark} ${snap}${item.key}${snap}`);
			});
		}
		...
		// }
		for (const key in likeWhere) {
			const snap = typeof likeWhere[key] === "string" ? '"' : "";
			likeArr.push(`\`${key}\` like ${snap}${likeWhere[key]}${snap}`);
		}
		const andStr = andArr.join(" and ");
		const orStr = orArr.join(" or ");
		const likeStr = likeArr.join(" and ");
		const likeStrS = andStr && likeStr.length > 1 ? `AND ${likeStr}` : likeStr;
		const betStr = betArr
			? `AND ${betArr[0]} BETWEEN ${betArr[1]} AND ${betArr[2]}`
			: "";

		const orderStr = orderByJson["type"]
			? `order by ${orderByJson["key"]} ${orderByJson["type"]}`
			: "";
		const limitStr = limitArr.length > 0 ? `limit ${limitArr.join(",")}` : "";
		const where = andStr || likeStrS || orStr || betStr ? "WHERE" : "";
		const sqlMod = `SELECT ${selectStr} FROM ${tableName} ${where} ${andStr} ${likeStrS} ${orStr} ${betStr} ${orderStr} ${limitStr}`;
		return new Promise((resolve, reject) => {
